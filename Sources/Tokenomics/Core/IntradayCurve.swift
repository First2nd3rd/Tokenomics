...
enum IntradayCurve {
    ...
    static func build(matrix: [String: [MinuteBucket]], now: Date, calendar: Calendar = .current) -> Series {
        let todayKey = DayBucket.dayKey(now, calendar: calendar)
        let comps = calendar.dateComponents([.hour, .minute], from: now)
        let nowMinute = min(1439, (comps.hour ?? 0) * 60 + (comps.minute ?? 0))

        let todayCum = prefixSum(matrix[todayKey] ?? Array(repeating: MinuteBucket(), count: 1440))
    ...
    private static func prefixSum(_ values: [MinuteBucket]) -> [Int] {
        var out = [Int](repeating: 0, count: values.count)
        var running = 0
        for i in values.indices { running += values[i].counts.total; out[i] = running }
        return out
    }
}
